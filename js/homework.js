console.log('Hellow from homework.js! ')
...
function getMovieInfo(movieName) {
    return new Promise((resolve, reject) => {
        console.log(`Searching for movie ${movieName} in the database...`)
...
function printMovieInfo(movieName) {
    getMovieInfo(movieName)
        .then(movie => console.log(movie.title, movie.director, movie.description, movie.runtime))
        .catch(err => console.warn(err))
};
...
let test = printMovieInfo('Indiana Jones and the Dark Knight');


// Example 2
// printMovieInfo('ET')
// Output: *Warning* ET cannot be accessed because it it too short
let test2 = printMovieInfo('ET');